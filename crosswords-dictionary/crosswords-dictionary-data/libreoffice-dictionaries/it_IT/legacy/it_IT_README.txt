...
dictionaryViewModel.lst; la directory dove trovare questo file � quella in cui dovete
installare il dizionario.

Aprite in un qualsiasi editor di testo il file dictionaryViewModel.lst che troverete
nella cartella citata precedentemente, in modo da poter inserire il codice
della lingua e della regione che vi interessa. 

Per esempio: 
nel vostro editor di testo aggiungete questa linea al file dictionaryViewModel.lst:
...
Campo 4: Il nome del dizionario da utilizzare "it_IT" o "it_CH" o ... 
	 ...
	 sufficiente aggiungere un'altra linea al file dictionaryViewModel.lst cambiando
...
The Italian dictionaryViewModel.
...
4. Others product that can use this dictionaryViewModel
5. Other spell checking program that can use this dictionaryViewModel
6. Thanks
...
The Italian dictionaryViewModel and affix file for the MySpell spell-checker used by
...
Warning: Since the release of OpenOffice.org 1.1 the Italian dictionaryViewModel is
...
dictionaryViewModel has been activate. You can do so simply by selecting from the menu
...
If the dictionaryViewModel is not activated yet, you can perform this action by selecting
...
dialog that will appear, you can activate the dictionaryViewModel.

In the case the dictionaryViewModel doesn't still work or you wish to upgarde the version
of your installed dictionaryViewModel you can follow the procedures explained below.


**************************************
3. How to install it in OpenOffice.org

Note: new dictionaryViewModel versions are avaiable from here:
http://sourceforge.net/projects/linguistico/

a) To install these dictionaries in OOo build 641c and greater:
   A. Unzip the dictionaryViewModel files, *.aff and *.dic,
      into your <OpenOffice.org>\user\wordbook\*.* directory

Attention: since OpenOffice.org 1.0.1 the right folder for the dictionaryViewModel is
           <OpenOffice.org>\share\dict\ooo

   B. Edit the dictionaryViewModel.lst file that is in that same directory
      using any text editor to register a dictionaryViewModel for a specific
      locale (the same dictionaryViewModel can be registered for multiple locales).

For example: 
To add the it_IT.zip dictionaryViewModel under the Italian (Italy) locale to OOo you
would:
cd <OpenOffice.org641>\user\wordbook (or <OpenOffice.org>\share\dict\ooo)
unzip it_IT.zip  


b) And then, using any text editor, add the following line to dictionaryViewModel.lst:
   DICT it IT it_IT
...
That it! Your dictionaryViewModel is installed and registered for that language.


**********************************************
4. Others product that can use this dictionaryViewModel

The Italian dictionaryViewModel can be used with many programs.
Here is some programs that can use this dictionaryViewModel:
...
* Mozilla-FireFox: with SpellBound extension
  spell checking when you type or upon request 
  Note: the dictionaryViewModel is installed under a different directory
  http://spellbound.sourceforge.net/
* Mozilla-ThunderBird: with "Thunderbird In-Line SpellChecker" extension
  spell checking when you type
  http://www.supportware.net/mozilla/#ext8

* vim: the MySpell dictionaryViewModel can be converted and used with the last vim
  version

Note: you can download dictionaryViewModel at mozilla site: http://www.mozdev.org/
...
This dictionaryViewModel can be used with different spell checking programs:
* MySpell. http://lingucomponent.openoffice.org
* Aspell. http://aspell.net
* HunSpell. http://hunspell.sf.net
...
evolving; simple misspelled words in a digital dictionaryViewModel can cause several
...
Community have actively took part into the quality check of this dictionaryViewModel
