...
public final class CachedDictionaryImpl implements CachedDictionary, SolverUpdateListener {
    ...
    public CachedDictionaryImpl(final Dictionary dictionary, final Collection<Slot> slots,
                                final Connectivity aConnectivity, final BacktrackHistoryReader history) {
        connectivity = aConnectivity;
        eligibility = (slot, word) -> slot.isCompatibleWith(word) &&
                !history.blacklist().getOrDefault(slot.uid(), Collections.emptySet()).contains(word);
     ...
     */
    private static DictionaryCache<Slot, String> populateCache(final Dictionary dictionary, final Collection<Slot> slots,
        ...
        for (final Slot slot : slots) {
             ...
             *
             * Avoid slow TreeSet, a simple list is sufficient for holding the sorted initial lookup result since it is
             * not modified.
             */
            final List<String> lookupResult = new ArrayList<>(dictionary.lookup(slot::isCompatibleWith));
    ...
    @Override
    public long refreshedCandidatesCount(final Slot wordVariable, final SlotIdentifier probedVariable) {
        ...
        if (connectivity.areSlotsConnected(wordVariable.uid(), probedVariable)) {
            count = candidates.stream().filter(word -> eligibility.test(wordVariable, word)).count();
        } else {
